...
# ─────────────────────────────────────────────────────────────────────────────
# _publish_after_write: sofortiges, gezieltes Update aus der Schreib-Antwort
# ─────────────────────────────────────────────────────────────────────────────

class TestPublishAfterWrite:
    """Nach einem Schreibvorgang werden W1/W2 sofort (ohne Poll) gepublished -
    mit der gleichen Formatierung wie der Poll (z.B. Schalter -> '2 ein')."""

    def _app(self):
        app = _make_full_app(
            adr_lookup={
                "0102500211": _entry("cc600_0102500211", "Bereg Giessdauer"),
                "0102500212": _entry("cc600_0102500212", "Bereg Handstart"),
            },
            html_adr_map={"FeldA": "0102500211", "FeldB": "0102500212"},
        )
        app._switch_enums = {"0102500212": {"0": "aus", "2": "ein"}}
        return app

    def _client(self):
        c = types.SimpleNamespace()
        c._initial_sensors = {
            "FeldA_Feld": {"value": "3", "unit": "min"},
            "FeldB_Feld": {"value": "0", "unit": ""},
        }
        return c

    def test_w1_und_w2_sofort_gepublished(self):
        app = self._app()
        app._publish_after_write(self._client(), "0102500211", {"w1": "5", "w2": "2"})
        states = {t: p for (t, p, r) in app._published if t.endswith("/state")}
        # Schalter (W2) bekommt die Enum-Textform aus dem Schaltertyp
        assert states["nersingen/sensor/cc600_0102500212/state"] == "2 ein"
        # W1 wird ebenfalls sofort aktualisiert
        assert "nersingen/sensor/cc600_0102500211/state" in states

    def test_leerer_arbeitswert_wird_uebersprungen(self):
        app = self._app()
        app._publish_after_write(self._client(), "0102500211", {"w1": "", "w2": "2"})
        states = [t for (t, p, r) in app._published if t.endswith("/state")]
        assert "nersingen/sensor/cc600_0102500212/state" in states
        assert "nersingen/sensor/cc600_0102500211/state" not in states
